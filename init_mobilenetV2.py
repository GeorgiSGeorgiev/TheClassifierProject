# Defines only one function which initialises the base MobileNetV2 model.
...
def build_model(num_classes, img_dim=224, img_augmentation=None):
    ...
    model = MobileNetV2(input_shape=input_shape, include_top=False, input_tensor=x, weights="imagenet")
    # We get the original pretrained MobileNetV2 model from the loaded inside Keras checkpoints which were trained
